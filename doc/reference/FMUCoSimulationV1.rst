...
.. py:class:: FMUCoSimulationV1([fmuDirUri,] modelIdentifier [, loggingOn=False, timeDiffResolution=1e-9])

   This class provides a basic Python wrapper that offers a set of convenient methods for accessing and manipulating FMUs for Co-Simulation according to the **FMI CS V1.0 standard**.
   Instances of class ``FMUCoSimulationV1`` own the actual FMU instance.
   ...
   .. code-block:: py
      ...
      fmu = fmipp.FMUCoSimulationV1(fmuDirUri, modelIdentifier, False, 1e-9)
   ...
   .. code-block:: py
      ...
      fmu = fmipp.FMUCoSimulationV1(modelIdentifier, False, 1e-9)
   ...
   .. py:method:: doStep(currentCommunicationPoint, communicationStepSize, newStep)
      ...
      :param bool newStep: if ``True``, the last communication step is accepted by the co-simulation algorithm and a new communication step is started
      :rtype: int (``statusOK``, ``statusWarning``, ``statusDiscard``, ``statusError`` or ``statusFatal``)
